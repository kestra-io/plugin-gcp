...
public abstract class AbstractList extends AbstractGcs implements ListInterface {
    @NotNull protected String from;

    @Schema(title = "If set to `true`, lists all versions of a blob. The default is `false`.")
    @PluginProperty(dynamic = true)
    private Boolean allVersions;

    @Builder.Default private final ListingType listingType = ListingType.DIRECTORY;

    protected String regExp;

    public Spliterator<com.google.cloud.storage.Blob> iterator(Storage connection, URI from) {
        Page<com.google.cloud.storage.Blob> list =
                connection.list(from.getAuthority(), options(from));
