...
@Schema(
        ...
        description =
                "This trigger will poll every `interval` a GCS bucket. You can search for all files"
                    + " in a bucket or directory in `from` or you can filter the files with a"
                    + " `regExp`.The detection is atomic, internally we do a list and interact only"
                    + " with files listed.\n"
                    + "Once a file is detected, we download the file on internal storage and"
                    + " processed with declared `action` in order to move or delete the files from"
                    + " the bucket (to avoid double detection on new poll)")
@Plugin(
        examples = {
            @Example(
...
public class Trigger extends AbstractTrigger
        implements PollingTriggerInterface,
                TriggerOutput<Downloads.Output>,
                GcpInterface,
                ListInterface,
                ActionInterface {
    ...
    public Optional<Execution> evaluate(ConditionContext conditionContext, TriggerContext context)
        ...
        java.util.List<Blob> list =
                run.getBlobs().stream()
                        .map(
                                throwFunction(
                                        blob -> {
                                            URI uri =
                                                    runContext.putTempFile(
                                                            Download.download(
                                                                    runContext,
                                                                    connection,
                                                                    BlobId.of(
        ...
        ExecutionTrigger executionTrigger =
                ExecutionTrigger.of(this, Downloads.Output.builder().blobs(list).build());
