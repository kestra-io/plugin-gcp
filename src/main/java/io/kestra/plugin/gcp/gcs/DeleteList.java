...
public class DeleteList extends AbstractList
        implements RunnableTask<DeleteList.Output>, ListInterface {
    ...
    public Output run(RunContext runContext) throws Exception {
        ...
        Flowable<com.google.cloud.storage.Blob> flowable =
                Flowable.create(
                        emitter -> {
        ...
        if (this.concurrent != null) {
            result =
                    flowable.parallel(this.concurrent)
                            .runOn(Schedulers.io())
            ...
            result =
                    flowable.filter(blob -> this.filter(blob, regExp))
                            .map(delete(logger, connection));
        }
        ...
        Pair<Long, Long> finalResult =
                result.reduce(
                                Pair.of(0L, 0L),
                                (pair, size) -> Pair.of(pair.getLeft() + 1, pair.getRight() + size))
                        .blockingGet();
        ...
        if (errorOnEmpty && finalResult.getLeft() == 0) {
            throw new NoSuchElementException(
                    "Unable to find any files to delete on '" + from + "'");
        }
