...
@Getter
@NoArgsConstructor
@Plugin(examples = {@Example(code = {"from: \"gs://my_bucket/dir/\""})})
@Schema(title = "Delete all file to a GCS bucket.")
public class DeleteList extends AbstractList implements RunnableTask<DeleteList.Output>, ListInterface {
    ...
    @Min(2)
    @Schema(title = "Number of concurrent parrallels deletion")
    ...
    public Output run(RunContext runContext) throws Exception {
        ...
        Flowable<com.google.cloud.storage.Blob> flowable = Flowable.create(emitter -> {
            this.iterator(connection, from).forEachRemaining(emitter::onNext);
            emitter.onComplete();
        ...
        if (this.concurrent != null) {
            result = flowable.parallel(this.concurrent).runOn(Schedulers.io()).filter(blob -> this.filter(blob, regExp))
                    .map(delete(logger, connection)).sequential();
        } else {
            result = flowable.filter(blob -> this.filter(blob, regExp)).map(delete(logger, connection));
        }

        Pair<Long, Long> finalResult =
                result.reduce(Pair.of(0L, 0L), (pair, size) -> Pair.of(pair.getLeft() + 1, pair.getRight() + size))
                        .blockingGet();
        ...
        return Output.builder().count(finalResult.getLeft()).size(finalResult.getRight()).build();
    }
