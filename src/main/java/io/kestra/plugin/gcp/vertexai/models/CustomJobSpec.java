...
public class CustomJobSpec {
    @Schema(title = "The spec of the worker pools including machine type and Docker image.",
    ...
    @Schema(title = "Specifies the service account for workload run-as account.",
    ...
    @Schema(title = "The full name of the Compute Engine [network](/compute/docs/networks-and-firewalls#networks) to which the Job should be peered.",
    ...
    @Schema(title = "The name of a Vertex AI Tensorboard resource to which this CustomJob",
    ...
    @Schema(title = "Whether you want Vertex AI to enable [interactive shell access](https://cloud.google.com/vertex-ai/docs/training/monitor-debug-interactive-shell) to training containers.")
    @PluginProperty(dynamic = false)
    private Boolean enableWebAccess;

    @Schema(title = "Scheduling options for a CustomJob.")
    @PluginProperty(dynamic = false)
    private Scheduling scheduling;

    @Schema(title = "The Cloud Storage location to store the output of this job.")
    @PluginProperty(dynamic = false)
    private GcsDestination baseOutputDirectory;

    public com.google.cloud.aiplatform.v1.CustomJobSpec to(RunContext runContext)
            throws IllegalVariableEvaluationException {
        com.google.cloud.aiplatform.v1.CustomJobSpec.Builder builder =
                com.google.cloud.aiplatform.v1.CustomJobSpec.newBuilder();

        this.workerPoolSpecs.stream().map(throwFunction(workerPoolSpec -> workerPoolSpec.to(runContext)))
