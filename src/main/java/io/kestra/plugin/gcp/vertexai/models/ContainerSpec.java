...
public class ContainerSpec {
    @Schema(
            title =
                    "The URI of a container image in the Container Registry that is to be run on"
                            + " each worker replica.",
            description =
                    "Must be on google container registry, example: `gcr.io/{{ project }}/{{ dir"
                            + " }}/{{ image }}:{{ tag }}`")
        ...
        return com.google.cloud.aiplatform.v1.ContainerSpec.newBuilder()
                ...
                .addAllCommand(
                        this.getCommands() != null
                                ? runContext.render(this.getCommands())
                                : List.of())
