...
public class ContainerSpec {
    @Schema(title = "The URI of a container image in the Container Registry that is to be run on each worker replica.",
            description = "Must be on google container registry, example: `gcr.io/{{ project }}/{{ dir }}/{{ image }}:{{ tag }}`")
    @PluginProperty(dynamic = true)
    @NotNull
    private String imageUri;

    @Schema(title = "The command to be invoked when the container is started.",
            description = "It overrides the entrypoint instruction in Dockerfile when provided.")
    @PluginProperty(dynamic = true)
    private List<String> commands;

    @Schema(title = "The arguments to be passed when starting the container.")
    @PluginProperty(dynamic = true)
    private List<String> args;

    public com.google.cloud.aiplatform.v1.ContainerSpec to(RunContext runContext)
        ...
        return com.google.cloud.aiplatform.v1.ContainerSpec.newBuilder()
                ...
                .addAllArgs(this.getArgs() != null ? runContext.render(this.getArgs()) : List.of()).build();
    }
}
