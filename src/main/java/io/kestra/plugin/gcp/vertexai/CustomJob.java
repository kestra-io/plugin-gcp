...
public class CustomJob extends AbstractTask implements RunnableTask<CustomJob.Output> {
    ...
    public Output run(RunContext runContext) throws Exception {
        ...
        JobServiceSettings pipelineServiceSettings =
                JobServiceSettings.newBuilder()
                        .setEndpoint(
                                runContext.render(this.region) + "-aiplatform.googleapis.com:443")
        ...
        try (JobServiceClient client = JobServiceClient.create(pipelineServiceSettings)) {
            com.google.cloud.aiplatform.v1.CustomJob.Builder builder =
                    com.google.cloud.aiplatform.v1.CustomJob.newBuilder()
                            .setJobSpec(this.getSpec().to(runContext))
                            .setDisplayName(jobName);

            LocationName parent =
                    LocationName.of(
                            runContext.render(this.projectId), runContext.render(this.region));

            com.google.cloud.aiplatform.v1.CustomJob response =
                    client.createCustomJob(parent, builder.build());
            ...
            try {
                tailThread =
                        LogTailService.tail(
                                ...
                                "resource.labels.job_id=\""
                                        + response.getName()
                                                .substring(response.getName().lastIndexOf("/") + 1)
                                        + "\" AND "
                ...
                if (this.wait) {
                    com.google.cloud.aiplatform.v1.CustomJob result =
                            Await.until(
                                    () -> {
                                        com.google.cloud.aiplatform.v1.CustomJob customJob =
                                                client.getCustomJob(response.getName());
                    ...
                    logger.info(
                            ...
                            result.getName(),
                            Duration.between(
                                    TimestampService.of(result.getCreateTime()),
