...
public class PythonPackageSpec {
    @Schema(title = "The Google Cloud Storage location of the Python package files which are the training program and its dependent packages.",
    ...
    @Schema(title = "Environment variables to be passed to the python module.", description = "Maximum limit is 100.")
    @PluginProperty(dynamic = true)
    @NotNull
    ...
    public com.google.cloud.aiplatform.v1.PythonPackageSpec to(RunContext runContext)
        ...
        com.google.cloud.aiplatform.v1.PythonPackageSpec.Builder builder =
        ...
        if (this.packageUris != null) {
            builder.addAllEnv(this.envs
                    .entrySet().stream().map(throwFunction(e -> EnvVar.newBuilder()
                            .setName(runContext.render(e.getKey())).setValue(runContext.render(e.getValue())).build()))
