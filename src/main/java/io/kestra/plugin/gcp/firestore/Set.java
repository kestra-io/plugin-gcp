...
@Getter
@NoArgsConstructor
@Schema(title = "Set a document in a collection.")
@Plugin(examples = {
        @Example(title = "Set a document from a map.",
                code = {"collection: \"persons\"", "document:", "  firstname: \"John\"", "  lastname: \"Doe\"",}),
        @Example(title = "Set a document from a JSON string.",
                code = {"collection: \"persons\"", "document: \"{{ outputs.task_id.data | json }}\""})})
public class Set extends AbstractFirestore implements RunnableTask<Set.Output> {
    @Schema(title = "The Firestore document.", description = "Can be a JSON string, or a map.")
    @PluginProperty(dynamic = true)
    private Object document;

    @Schema(title = "The Firestore document child path.")
    @PluginProperty(dynamic = true)
