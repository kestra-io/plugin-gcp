...
public class Query extends AbstractFirestore implements RunnableTask<FetchOutput> {
    ...
    private com.google.cloud.firestore.Query getQuery(
            RunContext runContext, CollectionReference collectionRef, List<Filter> filters)
    ...
    private com.google.cloud.firestore.Query appendQueryPart(
            RunContext runContext, com.google.cloud.firestore.Query query, Filter filter)
            throws IllegalVariableEvaluationException {
        switch (filter.getOperator()) {
            case EQUAL_TO:
                {
                    return query.whereEqualTo(
                            filter.getField(), runContext.render(filter.getValue()));
                }
            case NOT_EQUAL_TO:
                {
            ...
            case GREATER_THAN_OR_EQUAL_TO:
                ...
                // more where clause could be supported, but we need to validate that the value is a
                // List
        }
        // Should never occur
    ...
    private Pair<URI, Long> store(RunContext runContext, List<QueryDocumentSnapshot> documents)
            throws IOException {
    ...
    public static class Filter {
        ...
        @Schema(
                title =
                        "The operator for the filter, by default EQUAL_TO that will call"
                                + " 'collection.whereEqualTo(name, value)'")
        @PluginProperty(dynamic = false)
        @Builder.Default
