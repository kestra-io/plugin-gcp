...
public class TimePartitioning {
    ...
    @Schema(
            title =
                    "the number of milliseconds for which to keep the storage for a partition. When"
                            + " expired, the storage for the partition is reclaimed. If null, the"
                            + " partion does not expire.")
    @PluginProperty(dynamic = false)
    ...
    public static TimePartitioning of(com.google.cloud.bigquery.TimePartitioning timePartitioning) {
        return TimePartitioning.builder()
                ...
                .expiration(
                        timePartitioning.getExpirationMs() == null
                                ? null
