...
abstract public class AbstractBigquery extends AbstractTask {
    ...
    @Schema(title = "Automatic retry for retryable bigquery exceptions",
    ...
    @Builder.Default
    @Schema(title = "The reason that are valid for a automatic retry.")
    @PluginProperty(dynamic = true)
    protected List<String> retryReasons =
            Arrays.asList("rateLimitExceeded", "jobBackendError", "internalError", "jobInternalError");

    @Builder.Default
    @Schema(title = "The message that are valid for a automatic retry.",
            description = "Message is tested as a substring of the full message and case insensitive")
    @PluginProperty(dynamic = true)
    protected List<String> retryMessages =
            Arrays.asList("due to concurrent update", "Retrying the job may solve the problem");

    BigQuery connection(RunContext runContext) throws IllegalVariableEvaluationException, IOException {
        return connection(runContext, this.credentials(runContext), this.projectId, this.location);
    }

    static BigQuery connection(RunContext runContext, GoogleCredentials googleCredentials, String projectId,
            String location) throws IllegalVariableEvaluationException {
        VersionProvider versionProvider = runContext.getApplicationContext().getBean(VersionProvider.class);

        return BigQueryOptions.newBuilder().setCredentials(googleCredentials).setProjectId(runContext.render(projectId))
    ...
    protected Job waitForJob(Logger logger, Callable<Job> createJob, Boolean dryRun) {
        return Failsafe
                .with(AbstractRetry
                        .<Job>retryPolicy(this.getRetryAuto() != null ? this.getRetry()
                                : Exponential.builder().type("exponential").interval(Duration.ofSeconds(5))
                                        .maxInterval(Duration.ofMinutes(60)).maxDuration(Duration.ofMinutes(15))
                                        .maxAttempt(10).build())
                        .handleIf(throwable -> this.shouldRetry(throwable, logger))
                        .onFailure(event -> logger.error("Stop retry, attempts {} elapsed {} seconds",
                                event.getAttemptCount(), event.getElapsedTime().getSeconds(), event.getFailure()))
                        .onRetry(event -> {
                            logger.warn("Retrying, attempts {} elapsed {} seconds", event.getAttemptCount(),
                ...
                .get(() -> {
                    ...
                    try {
                        ...
                        if (exception instanceof com.google.cloud.bigquery.BigQueryException) {
                            com.google.cloud.bigquery.BigQueryException bqException =
                                    (com.google.cloud.bigquery.BigQueryException) exception;

                            logger.warn("Error query on {} with errors:\n[\n - {}\n]",
                                    job != null ? "job '" + job.getJobId().getJob() + "'" : "create job",
                                    String.join("\n - ", bqException.getErrors().stream().map(BigQueryError::toString)
                                            .toArray(String[]::new)));
                            ...
                            logger.warn("Error query on job '{}' with errors:\n[\n - {}\n]",
                                    job != null ? "job '" + job.getJobId().getJob() + "'" : "create job",
                                    String.join("\n - ", bqException.getErrors().stream().map(BigQueryError::toString)
                                            .toArray(String[]::new)));
