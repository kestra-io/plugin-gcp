...
@Getter
@NoArgsConstructor
public abstract class AbstractBigquery extends AbstractTask {
    @Schema(
    ...
    @Schema(
            ...
            description =
                    "Some exceptions (espacially rate limit) are not retried by default by BigQuery"
                        + " client, we use by default a transparent retry (not the kestra one) to"
                        + " handle this case.\n"
                        + "The default values are Exponential of 5 seconds for max 15 minutes and"
                        + " 10 attempts")
    @PluginProperty(dynamic = true)
    ...
    BigQuery connection(RunContext runContext)
                ...
                .get(
                        () -> {
                            ...
                            try {
                                ...
                                if (exception
                                        instanceof com.google.cloud.bigquery.BigQueryException) {
    ...
    private boolean shouldRetry(Throwable failure, Logger logger) {
        if (!(failure instanceof BigQueryException)) {
            logger.warn(
                    "Cancelled retrying, unknown exception type {}", failure.getClass(), failure);
            return false;
        }
