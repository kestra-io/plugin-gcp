...
abstract public class AbstractLoad extends AbstractBigquery implements RunnableTask<AbstractLoad.Output> {
    @Schema(title = "The table where to put query results", description = "If not provided a new table is created.")
    ...
    @Schema(title = "[Experimental] Automatic inference of the options and schema for CSV and JSON sources")
    @PluginProperty(dynamic = false)
    ...
    @Schema(title = "Whether BigQuery should allow extra values that are not represented in the table schema",
    ...
    @Schema(title = "The maximum number of bad records that BigQuery can ignore when running the job",
    ...
    @Schema(title = "The schema for the destination table",
    ...
    private List<Field> fields(List<Map<String, Object>> fields) {
        return fields.stream().map(r -> Field
                .newBuilder((String) r.get("name"), StandardSQLTypeName.valueOf((String) r.get("type")),
                        r.containsKey("fields") ? FieldList.of(fields((List<Map<String, Object>>) r.get("fields")))
                                : null)
                .setDescription(r.containsKey("description") ? (String) r.get("description") : null)
                .setMode(r.containsKey("mode") ? Field.Mode.valueOf((String) r.get("mode")) : null).build())
    ...
    protected Output outputs(RunContext runContext, LoadConfiguration configuration, Job job)
        ...
        return Output.builder().jobId(job.getJobId().getJob()).rows(stats.getOutputRows())
    ...
    private void metrics(RunContext runContext, JobStatistics.LoadStatistics stats, Job job)
        ...
        String[] tags = {"destination_table", runContext.render(this.destinationTable), "project_id",
                job.getJobId().getProject(), "location", job.getJobId().getLocation(),};
    ...
    public static class CsvOptions {
        @Schema(title = "Whether BigQuery should accept rows that are missing trailing optional columns",
        ...
        @Schema(title = "Whether BigQuery should allow quoted data sections that contain newline characters in a CSV file",
                description = "By default quoted newline are not allowed.")
        @PluginProperty(dynamic = true)
        private Boolean allowQuotedNewLines;

        @Schema(title = "The character encoding of the data",
        ...
        @Schema(title = "The separator for fields in a CSV file",
        ...
        @Schema(title = "The value that is used to quote data sections in a CSV file",
        ...
        @Schema(title = "The number of rows at the top of a CSV file that BigQuery will skip when reading the data",
    ...
    public static class AvroOptions {
        @Schema(title = "If Format option is set to AVRO, you can interpret logical types into their corresponding"
