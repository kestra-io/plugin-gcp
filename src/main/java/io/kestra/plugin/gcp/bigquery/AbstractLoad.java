...
@NoArgsConstructor
@LoadCsvValidation
public abstract class AbstractLoad extends AbstractBigquery
    ...
    @Schema(title = "The time partitioning type specification for the destination table")
    @PluginProperty(dynamic = true)
    @Builder.Default
    private TimePartitioning.Type timePartitioningType = TimePartitioning.Type.DAY;

    @Schema(title = "The action that should occur if the destination table already exists")
    @PluginProperty(dynamic = false)
    private JobInfo.WriteDisposition writeDisposition;

    @Schema(
            title =
                    "[Experimental] Automatic inference of the options and schema for CSV and JSON"
                            + " sources")
    @PluginProperty(dynamic = false)
    private Boolean autodetect;

    @Schema(title = "Whether the job is allowed to create tables")
    @PluginProperty(dynamic = false)
    private JobInfo.CreateDisposition createDisposition;

    @Schema(
            title =
                    "Whether BigQuery should allow extra values that are not represented in the"
                            + " table schema",
            description =
                    " If true, the extra values are ignored. If false, records with extra columns"
                        + " are treated as bad records, and if there are too many bad records, an"
                        + " invalid error is returned in the job result. By default unknown values"
                        + " are not allowed.")
    @PluginProperty(dynamic = false)
    ...
    @Schema(
            title =
                    "The maximum number of bad records that BigQuery can ignore when running the"
                            + " job",
            description =
                    " If the number of bad records exceeds this value, an invalid error is returned"
                            + " in the job result. By default no bad record is ignored.")
    @PluginProperty(dynamic = false)
    ...
    @Schema(
            ...
            description =
                    "The schema can be omitted if the destination table already exists, or if"
                        + " you're loading data from a Google Cloud Datastore backup (i.e. "
                        ...
                        + "```\n"
                        + "See type from"
                        + " [StandardSQLTypeName](https://javadoc.io/static/com.google.cloud/google-cloud-bigquery/1.88.0/com/google/cloud/bigquery/StandardSQLTypeName.html)")
    @PluginProperty(dynamic = false)
    private Map<String, Object> schema;

    @Schema(title = "The source format, and possibly some parsing options, of the external data")
    @PluginProperty(dynamic = false)
    private Format format;

    @Schema(title = "Csv parsing options")
    private CsvOptions csvOptions;

    @Schema(title = "Avro parsing options")
    ...
    private com.google.cloud.bigquery.Schema schema(Map<String, Object> schema) {
        ...
        if (!schema.containsKey("fields")) {
            throw new IllegalArgumentException(
                    "Unable to deserialize schema, no 'fields' with data '" + schema + "'");
        }

        return com.google.cloud.bigquery.Schema.of(
                fields((List<Map<String, Object>>) schema.get("fields")));
    }
                        ...
                        r ->
                                Field.newBuilder(
                                                ...
                                                r.containsKey("fields")
                                                        ...
                                                        : null)
                                        .setDescription(
                                                r.containsKey("description")
                                                        ? (String) r.get("description")
                                                        : null)
                                        .setMode(
                                                r.containsKey("mode")
                                                        ? Field.Mode.valueOf((String) r.get("mode"))
                                                        : null)
                                        .build())
    ...
    public static class Output implements io.kestra.core.models.tasks.Output {
        ...
        @Schema(title = "Destination table")
        private final String destinationTable;

        @Schema(title = "Output rows count")
    ...
    public static class CsvOptions {
        @Schema(
                title =
                        "Whether BigQuery should accept rows that are missing trailing optional"
                                + " columns",
                description =
                        "If true, BigQuery treats missing trailing columns as null values. If"
                            + " {@code false}, records with missing trailing columns are treated as"
                            + " bad records, and if there are too many bad records, an invalid"
                            + " error is returned in the job result. By default, rows with missing"
        ...
        @Schema(
                title =
                        "Whether BigQuery should allow quoted data sections that contain newline"
        ...
        @Schema(
                ...
                description =
                        "The supported values are UTF-8 or ISO-8859-1. The default value is UTF-8."
                            ...
                            + " using the values set in {@link #setQuote(String)} and {@link"
        ...
        @Schema(
                ...
                description =
                        "BigQuery converts the string to ISO-8859-1 encoding, and then uses the"
                            + " first byte of the encoded string to split the data in its raw,"
        ...
        @Schema(
                ...
                description =
                        "BigQuery converts the string to ISO-8859-1 encoding, and then uses the"
                            + " first byte of the encoded string to split the data in its raw,"
                            + " binary state. The default value is a double-quote ('\"'). If your"
                            + " data does not contain quoted sections, set the property value to an"
                            + " empty string. If your data contains quoted newline characters, you"
                            + " must also set {@link #setAllowQuotedNewLines(boolean)} property to"
                            + " {@code true}.")
        @PluginProperty(dynamic = true)
        ...
        private com.google.cloud.bigquery.CsvOptions to(RunContext runContext)
                throws IllegalVariableEvaluationException {
            com.google.cloud.bigquery.CsvOptions.Builder builder =
    ...
    public static class AvroOptions {
        @Schema(
                title =
                        "If Format option is set to AVRO, you can interpret logical types into"
                            + " their corresponding types (such as TIMESTAMP) instead of only using"
                            + " their raw types (such as INTEGER)",
