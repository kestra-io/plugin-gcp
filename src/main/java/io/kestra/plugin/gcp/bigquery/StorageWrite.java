...
@Getter
@NoArgsConstructor
@Plugin(examples = {@Example(code = {"from: \"{{ outputs.read.uri }}\"",
        "destinationTable: \"my_project.my_dataset.my_table\"", "writeStreamType: DEFAULT"})})
@Schema(title = "Load an kestra internal storage file on bigquery using "
...
public class StorageWrite extends AbstractTask implements RunnableTask<StorageWrite.Output> {
    ...
    @NotNull
    @NotEmpty
    @Schema(title = "The table where to load data", description = "The table must be created beefore.")
    private String destinationTable;

    @NotNull
    @NotEmpty
    @Builder.Default
    @Schema(title = "The type of write stream to use")
    private final WriteStreamType writeStreamType = WriteStreamType.DEFAULT;

    @NotNull
    @Builder.Default
    @Schema(title = "The number of records to send on each query")
    @PluginProperty(dynamic = false)
    protected final Integer bufferSize = 1000;

    @Schema(title = "The geographic location where the dataset should reside",
            description = "This property is experimental" + " and might be subject to change or removed.\n" + " \n"
    ...
    public Output run(RunContext runContext) throws Exception {
        ...
        try (BigQueryWriteClient connection = this.connection(runContext);
            ...
            try (JsonStreamWriter writer = this.jsonStreamWriter(runContext, parentTable, connection).build()) {
                Integer count = Flowable.create(FileSerde.reader(inputStream), BackpressureStrategy.BUFFER)
                        .map(this::map).buffer(this.bufferSize).map(list -> {
                ...
                Output.OutputBuilder builder = Output.builder().rows(count);
                ...
                if (this.writeStreamType == WriteStreamType.PENDING) {
                    ...
                    FinalizeWriteStreamResponse finalizeResponse =
                            connection.finalizeWriteStream(writer.getStreamName());
                    ...
                    if (!commitResponse.hasCommitTime()) {
                        ...
                        throw new Exception("Error on commit with error: " + commitResponse.getStreamErrorsList()
                                .stream().map(StorageError::getErrorMessage).collect(Collectors.joining("\n- ")));
                    } else {
    ...
    private Object transform(Object object) {
        if (object instanceof Map) {
            ...
            HashMap<Object, Object> map = value.entrySet().stream()
                    .map(e -> new AbstractMap.SimpleEntry<>(e.getKey(), transform(e.getValue())))
            ...
            return new JSONArray(value.stream().map(this::transform).collect(Collectors.toList()));
            ...
            return CivilTimeEncoder.encodePacked64TimeMicros(
                    org.threeten.bp.LocalTime.parse(value.format(DateTimeFormatter.ISO_LOCAL_TIME)));
    ...
    private BigQueryWriteClient connection(RunContext runContext)
        ...
        BigQueryWriteSettings bigQueryWriteSettings = BigQueryWriteSettings.newBuilder()
                ...
                .setQuotaProjectId(this.projectId).build();

        return BigQueryWriteClient.create(bigQueryWriteSettings);
    }

    private String[] tags(TableId tableId) {
        return new String[] {"write_stream_type", this.writeStreamType.name(), "project_id", tableId.getProject(),
                "dataset", tableId.getDataset(),};
    }

    private JsonStreamWriter.Builder jsonStreamWriter(RunContext runContext, TableName parentTable,
            BigQueryWriteClient client) throws IllegalVariableEvaluationException, IOException {
        if (this.writeStreamType == WriteStreamType.DEFAULT) {
            ...
            BigQuery bigQuery = AbstractBigquery.connection(runContext, this.credentials(runContext), this.projectId,
                    this.location);
            ...
            // https://cloud.google.com/bigquery/docs/write-api#write_to_a_stream_in_pending_mode
            // Write to a stream in committed mode :
            // https://cloud.google.com/bigquery/docs/write-api#write_to_a_stream_in_committed_mode

            WriteStream stream =
                    WriteStream.newBuilder().setType(WriteStream.Type.valueOf(this.writeStreamType.name())).build();
    ...
    public enum WriteStreamType {
        DEFAULT, COMMITTED, PENDING
    }

    @Builder
    @Getter
    public static class Output implements io.kestra.core.models.tasks.Output {
        @Schema(title = "Rows count")
        private final Integer rows;

        @Schema(title = "Rows count reported by BigQuery, only on `PENDING` writeStreamType")
        private final Long rowsCount;

        @Schema(title = "Commit time reported by BigQuery, only on `PENDING` writeStreamType")
