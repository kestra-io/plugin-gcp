...
@Getter
@NoArgsConstructor
public abstract class AbstractDataset extends AbstractBigquery
        implements RunnableTask<AbstractDataset.Output> {
    @NotNull
    @Schema(title = "The dataset's user-defined id")
    @PluginProperty(dynamic = true)
    protected String name;

    @Schema(title = "The dataset's access control configuration")
    ...
    @Schema(
            ...
            description =
                    "The minimum value is 3600000 milliseconds (one hour). Once this property is"
                        + " set, all newly-created tables in the dataset will have an"
                        + " expirationTime property set to the creation time plus the value in this"
                        + " property, and changing the value will only affect new tables, not"
                        ...
                        + " This property is experimental and might be subject to change or"
                        + " removed.")
    protected Long defaultTableLifetime;

    @Schema(title = "Description", description = "A user-friendly description for the dataset.")
    @PluginProperty(dynamic = true)
    protected String description;

    @Schema(title = "A user-friendly name for the dataset")
    @PluginProperty(dynamic = true)
    ...
    @Schema(
            ...
            description =
                    "Once this property is set, all newly-created partitioned tables in the dataset"
                            + " will have encryption key set to this value, unless table creation"
                            + " request (or query) overrides the key.")
    protected EncryptionConfiguration defaultEncryptionConfiguration;

    @Schema(
            title =
                    "[Optional] The default partition expiration time for all partitioned tables in"
                            + " the dataset, in milliseconds",
            description =
                    " Once this property is set, all newly-created partitioned tables in the "
                        + " dataset will has an expirationMs property in the timePartitioning"
                        + " settings set to this value. Changing the value only affect new tables,"
                        ...
                        + " overrides the use of defaultTableExpirationMs for partitioned tables:"
                        + " only one of defaultTableExpirationMs and defaultPartitionExpirationMs"
    ...
    @Schema(title = "The dataset's labels")
    @PluginProperty(dynamic = true)
                            ...
                            .collect(
                                    Collectors.toMap(
                                            ...
                                            Rethrow.throwFunction(
                                                    e -> runContext.render(e.getValue())))));
        }
    ...
    private Acl mapAcl(AccessControl accessControl) {
        ...
        switch (accessControl.getEntity().getType()) {
            case USER:
                return Acl.of(
                        new Acl.User(accessControl.getEntity().getValue()),
                        Acl.Role.valueOf(accessControl.getRole().name()));
            case GROUP:
                return Acl.of(
                        new Acl.Group(accessControl.getEntity().getValue()),
                        Acl.Role.valueOf(accessControl.getRole().name()));
            case DOMAIN:
                return Acl.of(
                        new Acl.Domain(accessControl.getEntity().getValue()),
                        Acl.Role.valueOf(accessControl.getRole().name()));
            case IAM_MEMBER:
                return Acl.of(
                        new Acl.IamMember(accessControl.getEntity().getValue()),
                        Acl.Role.valueOf(accessControl.getRole().name()));
            default:
                return null;
        }
    ...
    public static class Output implements io.kestra.core.models.tasks.Output {
        ...
        @NotNull
        @Schema(title = "A user-friendly description for the dataset")
