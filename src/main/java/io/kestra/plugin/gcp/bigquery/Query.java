...
@Getter
@NoArgsConstructor
@Plugin(examples = {@Example(title = "Create a table with a custom query", code = {
        "destinationTable: \"my_project.my_dataset.my_table\"", "writeDisposition: WRITE_APPEND", "sql: |", "  SELECT ",
        "    \"hello\" as string,", "    NULL AS `nullable`,", "    1 as int,", "    1.25 AS float,",
        "    DATE(\"2008-12-25\") AS date,", "    DATETIME \"2008-12-25 15:30:00.123456\" AS datetime,",
        "    TIME(DATETIME \"2008-12-25 15:30:00.123456\") AS time,",
        "    TIMESTAMP(\"2008-12-25 15:30:00.123456\") AS timestamp,", "    ST_GEOGPOINT(50.6833, 2.9) AS geopoint,",
        ...
        @Example(full = true, title = "Execute a query and fetch results sets on another task",
                code = {"tasks:", "- id: fetch", "  type: io.kestra.plugin.gcp.bigquery.Query", "  fetch: true",
                        "  sql: |", "    SELECT 1 as id, \"John\" as name", "    UNION ALL",
                        "    SELECT 2 as id, \"Doe\" as name", "- id: use-fetched-data",
                        "  type: io.kestra.core.tasks.debugs.Return", "  format: |", "    {{#each outputs.fetch.rows}}",
                        "    id : {{ this.id }}, name: {{ this.name }}", "    {{/each}}"})})
@Schema(title = "Execute BigQuery SQL query in a specific BigQuery database")
...
public class Query extends AbstractJob implements RunnableTask<Query.Output>, QueryInterface {
    ...
    @Schema(title = "The clustering specification for the destination table")
    @PluginProperty(dynamic = true)
    private List<String> clusteringFields;

    @Schema(title = "[Experimental] Options allowing the schema of the destination table to be updated as a side effect of the query job",
    ...
    @Schema(title = "The time partitioning field for the destination table.")
    @PluginProperty(dynamic = true)
    private String timePartitioningField;

    @Schema(title = "The time partitioning type specification.")
    @PluginProperty(dynamic = true)
    @Builder.Default
    private TimePartitioning.Type timePartitioningType = TimePartitioning.Type.DAY;

    @Schema(title = "Range partitioning field for the destination table.")
    @PluginProperty(dynamic = true)
    private String rangePartitioningField;

    @Schema(title = "The start of range partitioning, inclusive.")
    @PluginProperty(dynamic = true)
    private Long rangePartitioningStart;

    @Schema(title = "The end range partitioning, inclusive.")
    @PluginProperty(dynamic = true)
    private Long rangePartitioningEnd;

    @Schema(title = "The width of each interval.")
    @PluginProperty(dynamic = true)
    private Long rangePartitioningInterval;

    @Schema(title = "Sets the default dataset.",
            description = "This dataset is used for all unqualified table names used in the query.")
    @PluginProperty(dynamic = true)
    private String defaultDataset;

    @Schema(title = "Sets a priority for the query.")
    @PluginProperty(dynamic = false)
    @Builder.Default
    private QueryJobConfiguration.Priority priority = QueryJobConfiguration.Priority.INTERACTIVE;

    @Schema(title = "Sets whether the job is enabled to create arbitrarily large results.",
    ...
    @Schema(title = "Sets whether to look for the result in the query cache.",
    ...
    @Schema(title = "Sets whether nested and repeated fields should be flattened.",
            description = "If set to `false`, allowLargeResults must be `true`")
    @PluginProperty(dynamic = false)
    @Builder.Default
    private Boolean flattenResults = true;

    @Schema(title = "Sets whether to use BigQuery's legacy SQL dialect for this query.",
    ...
    @Schema(title = "Limits the billing tier for this job.",
    ...
    @Schema(title = "Limits the bytes billed for this job.",
    ...
    @Schema(title = "This is only supported in the fast query path.",
    ...
    public Query.Output run(RunContext runContext) throws Exception {
        ...
        Job queryJob = this.waitForJob(logger,
                () -> connection.create(
                        JobInfo.newBuilder(jobConfiguration).setJobId(BigQueryService.jobId(runContext, this)).build()),
                this.dryRun);
        ...
        Output.OutputBuilder output = Output.builder().jobId(queryJob.getJobId().getJob());
        ...
        if (this.fetch || this.fetchOne || this.store) {
            ...
            if (this.store) {
                ...
                output.uri(store.getKey()).size(store.getValue());
        ...
        if (tableIdentity != null) {
            DestinationTable destinationTable = new DestinationTable(tableIdentity.getProject(),
                    tableIdentity.getDataset(), tableIdentity.getTable());
    ...
    protected QueryJobConfiguration jobConfiguration(RunContext runContext) throws IllegalVariableEvaluationException {
        ...
        QueryJobConfiguration.Builder builder = QueryJobConfiguration.newBuilder(sql).setUseLegacySql(this.legacySql);
        ...
        if (this.timePartitioningField != null) {
            builder.setTimePartitioning(TimePartitioning.newBuilder(this.timePartitioningType)
                    .setField(runContext.render(this.timePartitioningField)).build());
        }
        ...
        if (this.rangePartitioningField != null) {
            builder.setRangePartitioning(RangePartitioning.newBuilder()
                    ...
                    .setRange(RangePartitioning.Range.newBuilder().setStart(this.rangePartitioningStart)
                            .setEnd(this.rangePartitioningEnd).setInterval(this.rangePartitioningInterval).build())
                    .build());
        }
    ...
    public static class Output implements io.kestra.core.models.tasks.Output {
        ...
        @Schema(title = "The destination table (if one) or the temporary table created automatically ")
        private DestinationTable destinationTable;
    }

    private String[] tags(JobStatistics.QueryStatistics stats, Job queryJob) {
        return new String[] {"statement_type", stats.getStatementType().name(), "fetch",
                this.fetch || this.fetchOne ? "true" : "false", "store", this.store ? "true" : "false", "project_id",
                queryJob.getJobId().getProject(), "location", queryJob.getJobId().getLocation(),};
    }

    public class DestinationTable {
        @Schema(title = "The project of the table")
        private String project;
        @Schema(title = "The dataset of the table")
    ...
    private List<Map<String, Object>> fetchResult(TableResult result) {
        return StreamSupport.stream(result.iterateAll().spliterator(), false)
                .map(fieldValues -> this.convertRows(result, fieldValues)).collect(Collectors.toList());
    }
    ...
    private Map.Entry<URI, Long> storeResult(TableResult result, RunContext runContext) throws IOException {
        ...
        try (OutputStream output = new FileOutputStream(tempFile);) {
            Flowable<Object> flowable = Flowable.create(s -> {
                StreamSupport.stream(result.iterateAll().spliterator(), false).forEach(fieldValues -> {
                    s.onNext(this.convertRows(result, fieldValues));
                });

                s.onComplete();
            }, BackpressureStrategy.BUFFER).doOnNext(row -> FileSerde.write(output, row));
            ...
            return new AbstractMap.SimpleEntry<>(runContext.putTempFile(tempFile), lineCount);
        }
    }
    ...
    private Object convertCell(Field field, FieldValue value, boolean isRepeated) {
        if (field.getMode() == Field.Mode.REPEATED && !isRepeated) {
            return value.getRepeatedValue().stream().map(fieldValue -> this.convertCell(field, fieldValue, true))
        ...
        if (LegacySQLTypeName.GEOGRAPHY.equals(field.getType())) {
            ...
            if (m.find()) {
                return Arrays.asList(Double.parseDouble(m.group(1)), Double.parseDouble(m.group(2)));
            }
        ...
        if (LegacySQLTypeName.RECORD.equals(field.getType())) {
            ...
            return field.getSubFields().stream()
                    .map(sub -> new AbstractMap.SimpleEntry<>(sub.getName(),
