        ...
        examples = {
            @Example(
                    ...
                    code = {
                        ...
                        "    STRUCT(4 AS x, 0 AS y, ARRAY(SELECT 1 UNION ALL SELECT 2 UNION ALL"
                                + " SELECT 3) AS z) AS `struct`"
                    }),
            @Example(
                    full = true,
...
public class Query extends AbstractJob implements RunnableTask<Query.Output>, QueryInterface {
    ...
    @Builder.Default private boolean legacySql = false;

    @Builder.Default private boolean fetch = false;

    @Builder.Default private boolean store = false;

    @Builder.Default private boolean fetchOne = false;
    ...
    @Schema(
            title =
                    "[Experimental] Options allowing the schema of the destination table to be"
            ...
            description =
                    " Schema update options are supported in two cases: * when writeDisposition is"
                        ...
                        + " partition of a table, specified by partition decorators. \n"
    ...
    @Schema(
            ...
            description =
                    "If `true` the query is allowed to create large results at a slight cost in"
                            + " performance. `destinationTable` must be provide")
    @PluginProperty(dynamic = false)
    ...
    @Schema(
            ...
            description =
                    "The query cache is a best-effort cache that will be flushed whenever tables in"
                            + " the query are modified. Moreover, the query cache is only available"
                            + " when `destinationTable` is not set ")
    @PluginProperty(dynamic = false)
    ...
    @Schema(
            ...
            description =
                    " A valid query will return a mostly empty response with some processing"
                        + " statistics, while an invalid query will return the same error it would"
                        + " if it wasn't a dry run.")
    @PluginProperty(dynamic = false)
    @Builder.Default
    ...
    @Schema(
            ...
            description =
                    "Queries that have resource usage beyond this tier will fail (without incurring"
    ...
    @Schema(
            ...
            description =
                    "The maximum number of rows of data to return per page of results. Setting this"
                            + " flag to a small value such as 1000 and then paging through results"
                            + " might improve reliability when the query result set is large. In"
                            + " addition to this limit, responses are also limited to 10 MB. By"
                            + " default, there is no maximum row count, and only the byte limit"
                            + " applies.")
    @PluginProperty(dynamic = false)
        ...
        if (tableIdentity != null) {
            DestinationTable destinationTable =
                    new DestinationTable(
                            tableIdentity.getProject(),
                            tableIdentity.getDataset(),
                            tableIdentity.getTable());
    ...
    protected QueryJobConfiguration jobConfiguration(RunContext runContext)
            throws IllegalVariableEvaluationException {
        String sql = runContext.render(this.sql);

        QueryJobConfiguration.Builder builder =
                QueryJobConfiguration.newBuilder(sql).setUseLegacySql(this.legacySql);

        if (this.clusteringFields != null) {
            builder.setClustering(
                    Clustering.newBuilder()
                            .setFields(runContext.render(this.clusteringFields))
                            .build());
        }
    ...
    public static class Output implements io.kestra.core.models.tasks.Output {
        ...
        @Schema(
                title =
                        "The destination table (if one) or the temporary table created"
                                + " automatically ")
    ...
    private void metrics(RunContext runContext, JobStatistics.QueryStatistics stats, Job queryJob)
        ...
        if (stats.getEstimatedBytesProcessed() != null) {
            runContext.metric(
                    Counter.of(
                            "estimated.bytes.processed", stats.getEstimatedBytesProcessed(), tags));
        }

        if (stats.getNumDmlAffectedRows() != null) {
            runContext.metric(
                    Counter.of("num.dml.affected.rows", stats.getNumDmlAffectedRows(), tags));
        }
        ...
        if (stats.getTotalBytesProcessed() != null) {
            runContext.metric(
                    Counter.of("total.bytes.processed", stats.getTotalBytesProcessed(), tags));
        }
        ...
        if (stats.getTotalPartitionsProcessed() != null) {
            runContext.metric(
                    Counter.of(
                            ...
                            stats.getTotalPartitionsProcessed(),
                            tags));
        }

        if (stats.getReferencedTables() != null) {
            runContext.metric(
                    Counter.of("referenced.tables", stats.getReferencedTables().size(), tags));
        }
        ...
        runContext.metric(
                Timer.of(
                        "duration",
                        Duration.ofMillis(stats.getEndTime() - stats.getStartTime()),
                        tags));
    }
    ...
    private Map.Entry<URI, Long> storeResult(TableResult result, RunContext runContext)
            throws IOException {
        // temp file
                    ...
                    Flowable.create(
                                    s -> {
                                        StreamSupport.stream(
                                                        result.iterateAll().spliterator(), false)
                                                .forEach(
                                                        fieldValues -> {
                                                            s.onNext(
                                                                    this.convertRows(
        ...
        result.getSchema()
                ...
                .forEach(
                        field -> {
                            row.put(
                                    field.getName(),
                                    convertCell(field, fieldValues.get(field.getName()), false));
                        });
    ...
    private Object convertCell(Field field, FieldValue value, boolean isRepeated) {
        ...
        if (LegacySQLTypeName.GEOGRAPHY.equals(field.getType())) {
            ...
            throw new IllegalFormatFlagsException(
                    "Couldn't match '" + value.getStringValue() + "'");
        }
                    ...
                    .map(
                            sub ->
                                    new AbstractMap.SimpleEntry<>(
                                            ...
                                            this.convertCell(
                                                    sub,
                                                    value.getRepeatedValue().get(counter.get()),
                                                    false)))
