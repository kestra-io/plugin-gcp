...
@Getter
@NoArgsConstructor
@Plugin(examples = {@Example(code = {"sourceTables:", "- \"my_project.my_dataset.my_table$20130908\"",
        "destinationTable: \"my_project.my_dataset.my_table\"",})})
@Schema(title = "Copy a BigQuery table or partition to other one")
...
public class Copy extends AbstractJob implements RunnableTask<Copy.Output> {
    @Schema(title = "The source tables", description = "Can be table or partitions.")
    @PluginProperty(dynamic = true)
    @NotNull
    private List<String> sourceTables;

    @Schema(title = "The destination table", description = "If not provided a new table is created.")
    @PluginProperty(dynamic = true)
    @NotNull
    private String destinationTable;

    @Schema(title = "Sets the supported operation types in table copy job. ",
    ...
    public Copy.Output run(RunContext runContext) throws Exception {
        ...
        logger.debug("Starting copy from {} to {}", jobConfiguration.getSourceTables(),
                jobConfiguration.getDestinationTable());

        Job copyJob = this.waitForJob(logger,
                () -> connection.create(
        ...
        Output.OutputBuilder output = Output.builder().jobId(copyJob.getJobId().getJob());

        return output.build();
    }

    protected CopyJobConfiguration jobConfiguration(RunContext runContext) throws IllegalVariableEvaluationException {
        CopyJobConfiguration.Builder builder = CopyJobConfiguration
                .newBuilder(BigQueryService.tableId(runContext.render(this.destinationTable)), runContext
                        .render(this.sourceTables).stream().map(BigQueryService::tableId).collect(Collectors.toList()));
    ...
    private String[] tags(JobStatistics.CopyStatistics stats, Job queryJob) {
        return new String[] {"project_id", queryJob.getJobId().getProject(), "location",
