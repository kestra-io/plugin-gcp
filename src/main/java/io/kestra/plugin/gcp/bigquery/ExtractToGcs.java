...
public class ExtractToGcs extends AbstractBigquery implements RunnableTask<ExtractToGcs.Output> {
    ...
    @Schema(
            title =
                    "The list of fully-qualified Google Cloud Storage URIs (e.g. gs://bucket/path)"
    ...
    @Schema(
            title =
                    "the compression value to use for exported files. If not set exported files "
                            + "are not compressed. ")
    @PluginProperty(dynamic = true)
    ...
    @Schema(
            title =
                    "The delimiter to use between fields in the exported data. By default \",\" is"
                            + " used.")
    @PluginProperty(dynamic = true)
    ...
    @Schema(
            ...
            description =
                    "The labels associated with this job. You can use these to organize and group"
                        + " your jobs. Label keys and values can be no longer than 63 characters,"
    ...
    @Schema(
            title =
                    "Whether to print out a header row in the results. By default an header is"
                            + " printed.")
    ...
    protected ExtractToGcs.Output execute(
            RunContext runContext, Logger logger, ExtractJobConfiguration configuration, Job job)
    ...
    private void metrics(RunContext runContext, JobStatistics.ExtractStatistics stats, Job job)
        ...
        if (stats.getDestinationUriFileCounts() != null) {
            ...
            long fileCounts =
                    stats.getDestinationUriFileCounts().stream().mapToLong(Long::longValue).sum();
        ...
        runContext.metric(
                Timer.of(
                        "duration",
                        Duration.ofNanos(stats.getEndTime() - stats.getStartTime()),
                        tags));
    }

    protected ExtractJobConfiguration buildExtractJob(RunContext runContext)
            throws IllegalVariableEvaluationException {
        ExtractJobConfiguration.Builder builder =
