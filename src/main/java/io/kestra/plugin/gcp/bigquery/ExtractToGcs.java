...
@Getter
@NoArgsConstructor
@Plugin(examples = {@Example(title = "Extract a BigQuery table to a gcs bucket",
        code = {"destinationUris: ", "  - \"gs://bucket_name/filename.csv\"",
                "sourceTable: \"my_project.my_dataset.my_table\"", "format: CSV", "fieldDelimiter: ';'",
                "printHeader: true"})})
@Schema(title = "Extract data from BigQuery table to GCS (Google Cloud Storage)")
public class ExtractToGcs extends AbstractBigquery implements RunnableTask<ExtractToGcs.Output> {

    @Schema(title = "The table to export.")
    @PluginProperty(dynamic = true)
    private String sourceTable;

    @Schema(title = "The list of fully-qualified Google Cloud Storage URIs (e.g. gs://bucket/path) where "
            + "the extracted table should be written.")
    @PluginProperty(dynamic = true)
    private List<String> destinationUris;

    @Schema(title = "the compression value to use for exported files. If not set exported files "
            + "are not compressed. ")
    @PluginProperty(dynamic = true)
    private String compression;

    @Schema(title = "The delimiter to use between fields in the exported data. By default \",\" is used.")
    @PluginProperty(dynamic = true)
    private String fieldDelimiter;

    @Schema(title = "The exported file format. If not set table is exported in CSV format. ")
    @PluginProperty(dynamic = true)
    private String format;

    @Schema(title = "[Optional] Flag if format is set to \"AVRO\".",
    ...
    @Schema(title = "[Optional] Job timeout in milliseconds. If this time limit is exceeded, "
            + "BigQuery may attempt to terminate the job.")
    private Long jobTimeoutMs;

    @Schema(title = "The labels associated with this job.",
    ...
    @Schema(title = "Whether to print out a header row in the results. By default an header is printed.")
    ...
    protected ExtractToGcs.Output execute(RunContext runContext, Logger logger, ExtractJobConfiguration configuration,
            Job job) throws InterruptedException, IllegalVariableEvaluationException, BigQueryException {
        ...
        return Output.builder().jobId(job.getJobId().getJob()).sourceTable(configuration.getSourceTable().getTable())
                .destinationUris(configuration.getDestinationUris()).fileCounts(stats.getDestinationUriFileCounts())
                .build();
    }
    ...
    public static class Output implements io.kestra.core.models.tasks.Output {
        ...
        @Schema(title = "source Table")
        private final String sourceTable;

        @Schema(title = "The destination URI file")
    ...
    private void metrics(RunContext runContext, JobStatistics.ExtractStatistics stats, Job job)
        ...
        String[] tags = {"source_table", runContext.render(this.sourceTable), "project_id", job.getJobId().getProject(),
    ...
    protected ExtractJobConfiguration buildExtractJob(RunContext runContext) throws IllegalVariableEvaluationException {
        ExtractJobConfiguration.Builder builder = ExtractJobConfiguration.newBuilder(
                BigQueryService.tableId(runContext.render(this.sourceTable)), runContext.render(this.destinationUris));
