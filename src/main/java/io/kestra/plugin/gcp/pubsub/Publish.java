...
@Plugin(examples = {@Example(code = {"topic: topic-test", "from:",
        "-  data: {{ 'base64-encoded-string-1' | base64encode }}", "   attributes:", "       testAttribute: KestraTest",
        "   messageId: '1234'", "   orderingKey: 'foo'", "-  data: {{ 'base64-encoded-string-2' | base64encode }}",
        "-  attributes:", "       testAttribute: KestraTest"})})
public class Publish extends AbstractPubSub implements RunnableTask<Publish.Output> {
    @PluginProperty(dynamic = true)
    @NotNull
    @Schema(title = "The source of the published data.",
