    ...
    public Output run(RunContext runContext) throws Exception {
        if (this.maxDuration == null && this.maxRecords == null) {
            throw new IllegalArgumentException(
                    "'maxDuration' or 'maxRecords' must be set to avoid an infinite loop");
        }

        var subscriptionName =
                this.createSubscription(runContext, subscription, autoCreateSubscription);
        ...
        try (var outputFile = new BufferedOutputStream(new FileOutputStream(tempFile))) {
            ...
            runContext.metric(
                    Counter.of(
                            "records", total.get(), "topic", runContext.render(this.getTopic())));
            outputFile.flush();
        }
    ...
    private boolean ended(AtomicInteger count, ZonedDateTime start) {
        ...
        if (this.maxDuration != null
                && ZonedDateTime.now().toEpochSecond()
                        > start.plus(this.maxDuration).toEpochSecond()) {
            return true;
        }
