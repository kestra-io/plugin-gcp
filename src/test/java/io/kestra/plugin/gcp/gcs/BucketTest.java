...
class BucketTest {
    ...
    private RunContext runContext(String bucketId) {
        return runContextFactory.of(ImmutableMap.of("project", this.project, "bucket", bucketId));
    }

    private CreateBucket.CreateBucketBuilder<?, ?> createBuilder() {
        return CreateBucket.builder().id(BucketTest.class.getSimpleName()).type(CreateBucket.class.getName())
    ...
    void createNoException() throws Exception {
        CreateBucket task = createBuilder().indexPage("createUpdate").ifExists(CreateBucket.IfExists.SKIP).build();
    ...
    void createUpdate() throws Exception {
        CreateBucket task = createBuilder().indexPage("createUpdate").ifExists(CreateBucket.IfExists.UPDATE).build();
    ...
    void update() throws Exception {
        UpdateBucket task =
                UpdateBucket.builder().id(UpdateBucket.class.getSimpleName()).type(CreateBucket.class.getName())
                        .name("{{bucket}}").projectId("{{project}}").indexPage("update").build();
    ...
    void acl() throws Exception {
        CreateBucket task = createBuilder().indexPage("createUpdate")
                .acl(Collections.singletonList(AccessControl.builder()
                        .entity(Entity.builder().type(Entity.Type.USER)
                                .value("kestra-unit-test@kestra-unit-test.iam.gserviceaccount.com").build())
                        .role(AccessControl.Role.OWNER).build()))
                .ifExists(CreateBucket.IfExists.UPDATE).build();
        ...
        assertThat(bucket.getAcl(), hasItems(matchingAcl(
    ...
    void iamPolicy() throws Exception {
        CreateBucketIamPolicy.CreateBucketIamPolicyBuilder<?, ?> builder = CreateBucketIamPolicy.builder()
                .id(UpdateBucket.class.getSimpleName()).type(CreateBucket.class.getName()).name("{{bucket}}")
                .projectId("{{project}}").member("domain:kestra.io").role("roles/storage.objectViewer");
    ...
    void delete() throws Exception {
        ...
        DeleteBucket task = DeleteBucket.builder().id(BucketTest.class.getSimpleName())
                .type(DeleteBucket.class.getName()).name("{{bucket}}").projectId("{{project}}").build();
        ...
        task = DeleteBucket.builder().id(BucketTest.class.getSimpleName()).type(DeleteBucket.class.getName())
                .name("{{bucket}}").projectId("{{project}}").build();
    ...
    private void createBucketWithLifecycleRule(RunContext rc, java.util.List<BucketLifecycleRule> rules)
            throws Exception {
        CreateBucket task = createBuilder().lifecycleRules(rules).ifExists(CreateBucket.IfExists.ERROR).build();
        ...
        java.util.List<Matcher<LifecycleRule>> matchers = task.getLifecycleRules().stream()
                .map(rule -> matchingLifecycleRuleOnTypeAndAge(rule.convert())).collect(Collectors.toList());

        assertThat(matchers.size(), is(rules.size()));

        assertThat(bucket.getLifecycleRules(), hasItems(matchers.toArray(new Matcher[0])));
    ...
    void createBucketWithDeleteLifecycleRule() throws Exception {
        ...
        createBucketWithLifecycleRule(rc, Collections.singletonList(BucketLifecycleRule.builder()
                .condition(BucketLifecycleRule.Condition.builder().age(1).build())
                .action(BucketLifecycleRule.Action.builder().type(BucketLifecycleRule.Action.Type.DELETE).build())
                .build()));
    }
    ...
    void createBucketWithSetStorageClassLifecycleRule() throws Exception {
        ...
        RunContext rc = runContext(bucketId);
        createBucketWithLifecycleRule(rc, ImmutableList.of(
                BucketLifecycleRule.builder().condition(BucketLifecycleRule.Condition.builder().age(30).build())
                        .action(BucketLifecycleRule.Action.builder()
                                .type(BucketLifecycleRule.Action.Type.SET_STORAGE_CLASS)
                                .value(io.kestra.plugin.gcp.gcs.models.StorageClass.NEARLINE.name()).build())
                        .build(),

                BucketLifecycleRule.builder().condition(BucketLifecycleRule.Condition.builder().age(60).build())
                        .action(BucketLifecycleRule.Action.builder()
                                .type(BucketLifecycleRule.Action.Type.SET_STORAGE_CLASS)
                                .value(io.kestra.plugin.gcp.gcs.models.StorageClass.COLDLINE.name()).build())
                        .build(),

                BucketLifecycleRule.builder().condition(BucketLifecycleRule.Condition.builder().age(90).build())
                        .action(BucketLifecycleRule.Action.builder()
                                .type(BucketLifecycleRule.Action.Type.SET_STORAGE_CLASS)
                                .value(io.kestra.plugin.gcp.gcs.models.StorageClass.ARCHIVE.name()).build())
                        .build(),

                BucketLifecycleRule.builder().condition(BucketLifecycleRule.Condition.builder().age(1).build()).action(
                        BucketLifecycleRule.Action.builder().type(BucketLifecycleRule.Action.Type.DELETE).build())
                        .build()));
    }
    ...
    private Matcher<Acl> matchingAcl(Acl expected) {
        return new TypeSafeMatcher<>() {
            ...
            protected boolean matchesSafely(Acl actual) {
                return actual.getEntity().equals(expected.getEntity()) && actual.getRole().equals(expected.getRole());
            }
        };
    }
        ...
        return new TypeSafeMatcher<LifecycleRule>() {
            ...
            protected boolean checkSetStorageRule(LifecycleRule actual) {
                ...
                StorageClass actualStorageClass =
                        ((LifecycleRule.SetStorageClassLifecycleAction) actual.getAction()).getStorageClass();
                StorageClass expectedStorageClass =
                        ((LifecycleRule.SetStorageClassLifecycleAction) expected.getAction()).getStorageClass();
