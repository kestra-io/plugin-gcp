...
class QueryTest {
    ...
    void fetch() throws Exception {
        RunContext runContext = runContextFactory.of(ImmutableMap.of("sql", sql(), "flow",
                ImmutableMap.of("id", FriendlyId.createFriendlyId(), "namespace", "io.kestra.tests"), "execution",
                ImmutableMap.of("id", FriendlyId.createFriendlyId()), "taskrun",
                ImmutableMap.of("id", FriendlyId.createFriendlyId())));

        Query task = Query.builder().sql("{{sql}}").location("EU").fetch(true).build();
        ...
        assertThat((List<Long>) ((Map<String, Object>) rows.get(0).get("struct")).get("z"),
    ...
    void store() throws Exception {
        Query task = Query.builder().id(QueryTest.class.getSimpleName()).type(Query.class.getName())
                .sql(sql() + "\n UNION ALL \n " + sql()).store(true).build();

        Query.Output run = task.run(TestsUtils.mockRunContext(runContextFactory, task, ImmutableMap.of()));

        assertThat(CharStreams.toString(new InputStreamReader(storageInterface.get(run.getUri()))),
    ...
    void destination() throws Exception {
        ...
        Query task = Query.builder().id(QueryTest.class.getSimpleName()).type(Query.class.getName())
                .sql("{% for input in inputs.loop %}" + "SELECT" + "  \"{{execution.id}}\" as execution_id,"
                        + "  TIMESTAMP \"{{execution.startDate | date(\"yyyy-MM-dd HH:mm:ss.SSSSSS\")}}\" as execution_date,"
                        + "  {{ input }} as counter" + "{{ loop.last  == false ? '\nUNION ALL\n' : '\n' }}"
                        + "{% endfor %}")
                .destinationTable(project + "." + dataset + "." + friendlyId).timePartitioningField("execution_date")
                ...
                .writeDisposition(JobInfo.WriteDisposition.WRITE_APPEND).build();

        RunContext runContext = TestsUtils.mockRunContext(runContextFactory, task,
                ImmutableMap.of("loop", ContiguousSet.create(Range.closed(1, 25), DiscreteDomain.integers())));
    ...
    void error() {
        Query task = Query.builder().id(QueryTest.class.getSimpleName()).type(Query.class.getName())
                .sql("SELECT * from `{{execution.id}}`").build();
    ...
    void script() throws Exception {
        Query task = Query.builder().id(QueryTest.class.getSimpleName()).type(Query.class.getName())
                .sql("{% for input in inputs.loop %}" + "SELECT" + "  \"{{execution.id}}\" as execution_id,"
                        + "  TIMESTAMP \"{{execution.startDate | date(\"yyyy-MM-dd HH:mm:ss.SSSSSS\") }}\" as execution_date,"
                        + "  {{input}} as counter;" + "{% endfor %}")
                .build();

        RunContext runContext = TestsUtils.mockRunContext(runContextFactory, task,
                ImmutableMap.of("loop", ContiguousSet.create(Range.closed(1, 2), DiscreteDomain.integers())));
    ...
    void retry() throws Exception {
        ...
        for (int i = 0; i < 50; i++) {
            Query task = Query.builder().id(QueryTest.class.getSimpleName()).type(Query.class.getName())
                    .sql("SELECT \"" + i + "\" as value").destinationTable(table)
                    .createDisposition(JobInfo.CreateDisposition.CREATE_IF_NEEDED)
                    .writeDisposition(JobInfo.WriteDisposition.WRITE_TRUNCATE).build();
        ...
        List<Query.Output> results = futures.stream().map(outputFuture -> {
            try {
    ...
    void scriptError() {
        Query task = Query.builder().id(QueryTest.class.getSimpleName()).type(Query.class.getName())
                .sql("{% for input in  inputs.loop %}" + "SELECT * from `{{execution.id}}`;" + "{% endfor %}").build();
    ...
    void scriptMultipleNoOutput() throws Exception {
        Query task = Query.builder().id(QueryTest.class.getSimpleName()).type(Query.class.getName())
                .sql("DROP TABLE IF EXISTS `" + project + "." + this.dataset + ".not`;" + "DROP TABLE IF EXISTS `"
                        + project + "." + this.dataset + ".exist`;")
                .build();
    ...
    void concurrency() throws Exception {
        ...
        Query create = Query.builder().id(QueryTest.class.getSimpleName()).type(Query.class.getName())
                .sql("CREATE TABLE " + table + " AS SELECT 1 AS number").build();
        ...
        Query task = Query.builder().id("test").type(Query.class.getName()).sql("SELECT * FROM " + table + ";")
                .fetchOne(true).build();
        RunContext runContext = TestsUtils.mockRunContext(runContextFactory, task, ImmutableMap.of());
