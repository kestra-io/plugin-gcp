...
class QueryTest {
    ...
    static String sql() {
        return "SELECT \n"
                ...
                + "  STRUCT(NULL as v, 4 AS x, 0 AS y, ARRAY(SELECT 1 UNION ALL SELECT 2 UNION"
                + " ALL SELECT 3) AS z) AS `struct`";
    }
    ...
    void fetch() throws Exception {
        ...
        assertThat(
                (List<Long>) ((Map<String, Object>) rows.get(0).get("struct")).get("z"),
    ...
    void fetchLongPage() throws Exception {
        Query task =
                Query.builder()
                        ...
                        .type(Query.class.getName())
                        .sql(
                                "SELECT repository_forks FROM"
                                        + " `bigquery-public-data.samples.github_timeline` LIMIT"
                                        + " 100000")
                        .fetch(true)
                        .build();
    ...
    void destination() throws Exception {
        ...
        Query task =
                Query.builder()
                        ...
                        .type(Query.class.getName())
                        .sql(
                                "{% for input in inputs.loop %}SELECT  \"{{execution.id}}\" as"
                                    + " execution_id,  TIMESTAMP \"{{execution.startDate |"
                                    + " date(\"yyyy-MM-dd HH:mm:ss.SSSSSS\")}}\" as execution_date,"
                ...
                TestsUtils.mockRunContext(
                        ...
                        ImmutableMap.of(
                                ...
                                ContiguousSet.create(
                                        Range.closed(1, 25), DiscreteDomain.integers())));
    ...
    void error() {
        ...
        RunContext runContext =
                TestsUtils.mockRunContext(runContextFactory, task, ImmutableMap.of());
        ...
        Query task =
                Query.builder()
                        ...
                        .sql(
                                "{% for input in inputs.loop %}SELECT  \"{{execution.id}}\" as"
                                        + " execution_id,  TIMESTAMP \"{{execution.startDate |"
                                        + " date(\"yyyy-MM-dd HH:mm:ss.SSSSSS\") }}\" as"
                                        + " execution_date,  {{input}} as counter;{% endfor %}")
                        .build();
        ...
        RunContext runContext =
                TestsUtils.mockRunContext(
                        ...
                        ImmutableMap.of(
                                "loop",
                                ContiguousSet.create(
                                        Range.closed(1, 2), DiscreteDomain.integers())));
