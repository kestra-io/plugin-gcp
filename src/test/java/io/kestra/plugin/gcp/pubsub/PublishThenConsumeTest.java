...
class PublishThenConsumeTest {
    ...
    @Inject protected StorageInterface storageInterface;
    ...
    private URI createTestFile(RunContext runContext) throws Exception {
        ...
        FileSerde.write(
                output,
                Message.builder()
                        .data(Base64.getEncoder().encodeToString("Hello World".getBytes()))
                        .build());
        FileSerde.write(output, Message.builder().attributes(Map.of("key", "value")).build());
        return storageInterface.put(
                URI.create("/" + IdUtils.create() + ".ion"), new FileInputStream(tempFile));
    }
}
