...
// The interactive Storybook resolves expressions through the expressions SDK mock via
// .storybook/main.ts viteFinal alias; that alias doesn't apply under vitest, so reuse the same mock
// module here at the import boundary. Single source of truth for both. Outputs/metrics no longer go
// through the SDK — the stories supply them directly via fetchOutputs/fetchMetrics props.
